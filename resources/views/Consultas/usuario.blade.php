...
@section('titulo','Consulta de usuário')
...
@section('conteudo')
    ...
    <div class="container mt-5">
        <div class="d-flex justify-content-center align-items-center text-center"> 
            ...
            {{-- msg para quando a pessoa clicar em editar o registro --}}
            <div id="edicaoRegistro" hidden>
                @include('layouts.msg',['tipoMsg' => 'warning-edit', 'msg' => 'Atenção você habilitou a edição de registro!!'])
            </div>

        </div>
        
        <h3 class="titulo-consultas">Consulta de Usuário</h3>

        {{-- VALIDAÇÃO PAR QUANDO NÃO TIVER REGISTROS NA TABELA --}}
        @if (!$rows->isEmpty())
            
            <div class="table-responsive">
                <table class="table table-striped caption-top tabela-personalizada">
                    ...
                    <tbody>
                        ...
                        <form action="{{ route('edit.registro', ['tabela' => 'tbusuarios', 'id' => $item->id]) }}" method="POST" >
                            @csrf @method('PUT')
                            ...
                            <tr id="{{$item->id}}">
                                ...
                                <td><input type="text" class="inputs-edit" name='sobrenome' value="{{ $item->sobrenome }}" readonly></td>
                                <td><input type="text" class="inputs-edit" name='cpf' value="{{ $item->cpf }}" readonly></td>
                                <td><input type="text" class="inputs-edit" name='rg' value="{{ $item->rg }}" readonly></td>
                                <td><input type="text" class="inputs-edit" name='data_nascimento' value="{{ $item->data_nascimento }}" readonly></td>

                                <td>
                                    <div class="acoes">
 
                                        {{-- icone editar --}}
                                        <button type="button" class="acao acoes-editar" name="{{$item->id}}" id="edit-{{$item->id}}">
                                        ...
                                        {{-- icone confirmar --}}
                                        <button type="submit" class="acao acoes-editar-confirma" name="{{$item->id}}" id="confirma-{{$item->id}}" hidden>
            ...
            <div class="modal fade" id="modalConfirmaExclusao" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog ">
                    <div class="modal-content modal-confirmacao">
                        ...
                        <form action="{{ route('delete',['tabela' => 'tbusuarios']) }}" method="POST">
                            @csrf @method("DELETE")
